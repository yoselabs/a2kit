...
- **Spec-drift-gate allowlist: second reconcile pass.** `tests/test_spec_symbol_drift.py` (the `spec-drift-gate` capability, ADR 0018) landed with ~65 grandfathered-drift entries. The `reconcile-stale-specs` change cut structural rot (39 requirements removed, 27 modernised) and brought the allowlist down to ~35 `# reconcile:` entries. The residual has two causes: (a) reconciled spec bodies still cite removed names to *document* migrations — the ADR 0018 D7.2 anti-pattern, which means rewriting those sentences to describe only live surface; (b) ~7 drifted specs were never in `reconcile-stale-specs`' scope (`otel-adapter`, `di-conditional-injection`, `lazy-init-resources`, `verb-decorators`, `test-container-peek`, `app-builder-runtime`, `di-scope-cleanup-stack`). Trigger: pick up a `reconcile-stale-specs-2` change that clears the remaining `# reconcile:` tags. `app-builder-runtime` specifically should be deleted, not repaired (its capability is superseded — ADR 0018 D7.3).
