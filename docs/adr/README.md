...
Every ADR begins with a YAML frontmatter block. The frontmatter is the
**agent-grep layer**: structured metadata an LLM (or a script) can read
without parsing the body. It also drives `docs/adr/INDEX.md` (auto-
generated by `scripts/adr_index.py`). Schema lives at
`docs/adr/schema.json` and is enforced on every commit.

```yaml
---
id: "0042"                       # zero-padded, matches filename prefix
status: accepted                 # proposed | accepted | deprecated | superseded
date: 2026-05-18                 # original acceptance date, never changes
last_reviewed: 2026-05-18        # bumped on periodic re-validation
supersedes: []                   # list of ADR ids this replaces, [] if none
superseded_by: null              # set to the replacing ADR id when superseded
tags: [surface, packaging]       # kebab-case, lowercase, for filtering
deciders: [Denis Tomilin]        # who signed off
---
```

`scripts/adr_index.py` validates this block against `schema.json`,
extracts the Y-statement from the body, and regenerates `INDEX.md`.
Run `make adr-index` after authoring or editing an ADR (pre-commit
enforces freshness; `make adr-check` is the read-only equivalent).
...
- **Status updates.** When a new ADR supersedes an old one, edit the
  ...
  (YYYY-MM-DD)`, set `status: superseded` and `superseded_by: "NNNN"`
  in the frontmatter, and run `make adr-index`. This is the only
  allowed content edit to an accepted ADR.
- **INDEX regeneration.** `docs/adr/INDEX.md` is auto-generated from
  frontmatter by `scripts/adr_index.py`. Never edit it by hand —
  changes will be overwritten on the next `make adr-index` (or
  pre-commit). The INDEX is the agent-loadable entry point: load it
  once to see every recorded decision, then follow links to the full
  bodies. The corollary: keep frontmatter (especially `tags` and the
  body's Y-statement) current — that's what an agent reads to decide
  which ADR is relevant before loading the body.
