...
```
scripts/extract_facts.py       Walks src/**/*.py, emits curated JSON
                               (functions, modules, suppressions).
                               Pure function of input tree.
policies/                      Rego policy bundle (.rego files).
  ...
  data.json                    Per-policy allowlist entries with
                               ...
                               name_collision: [...]}}}. Loaded into
                               `data.a2kit.allowlist` when the bundle
                               is evaluated.
...
OPA is pinned to a specific version in the `Makefile`. The
`bootstrap:` target verifies it; install yourself first.
