...
The closure lint rule (legacy code `A2K-RAISES-CLOSURE`, renamed to the ruff-`noqa`-grammar-safe code `AK220` matching `^[A-Z]+[0-9]+$`) SHALL fire when a tool body (or service method body, when the method's return is `Annotated[T, Raises(...)]`) contains a `raise X(...)` statement where `X` is NOT in:
...
The rule SHALL produce `LintMessage` with severity `error` (not warning). Strict mode is the only mode in v1. The legacy `A2K-RAISES-CLOSURE` spelling resolves to `AK220` through `LEGACY_CODE_ALIASES` during the deprecation window.
...
- **WHEN** lint runs
- **THEN** `AK220` fires at the `raise InvalidId(...)` line with severity `error`
...
- **WHEN** lint runs
- **THEN** no `AK220` message fires for this body
...
- **WHEN** lint runs
- **THEN** no `AK220` fires (translation chain covers the raise)

### Requirement: `A2K-RAISES-UNCOVERED` flags registry-known throwers without coverage

The uncovered-call lint rule (legacy code `A2K-RAISES-UNCOVERED`, renamed to the ruff-`noqa`-grammar-safe code `AK221`) SHALL fire when a tool body contains a call expression to a function listed in the `raises_registry` whose declared raise type is NOT covered by:
...
The rule SHALL produce `LintMessage` with severity `warning` by default (uncovered calls are not strictly wrong but indicate a likely missing translation). The legacy `A2K-RAISES-UNCOVERED` spelling resolves to `AK221` through `LEGACY_CODE_ALIASES` during the deprecation window.
...
- **WHEN** lint runs
- **THEN** `AK221` warning fires at the call site naming `client.get` and the registry-declared raises
...
- **WHEN** lint runs
- **THEN** no `AK221` warning fires for that line

### Requirement: `A2K-RAISES-NOT-TYPED` rejects non-AppError members

The not-typed lint rule (legacy code `A2K-RAISES-NOT-TYPED`, renamed to the ruff-`noqa`-grammar-safe code `AK222`) SHALL fire when a `Raises(...)` marker contains a type that is not a subclass of `AppError`. The rule SHALL produce `LintMessage` with severity `error`. The legacy `A2K-RAISES-NOT-TYPED` spelling resolves to `AK222` through `LEGACY_CODE_ALIASES` during the deprecation window.

#### Scenario: Raises with raw asyncpg type fires

- **GIVEN** an annotation `Annotated[Memory, Raises(asyncpg.PostgresError)]`
- **WHEN** lint runs
- **THEN** `AK222` fires at the annotation naming `asyncpg.PostgresError`
...
The a2effect package SHALL declare its lint rules under the `a2lint.rules` entry-point group in its `pyproject.toml`, keyed by the ruff-`noqa`-grammar-safe codes (`AK220`, `AK221`, `AK222`) matching `^[A-Z]+[0-9]+$`:

```toml
[project.entry-points."a2lint.rules"]
"AK220" = "a2effect._lint:raises_closure_rule"
"AK221" = "a2effect._lint:raises_uncovered_rule"
"AK222" = "a2effect._lint:raises_not_typed_rule"
```
...
- **THEN** the three rules `AK220`, `AK221`, `AK222` are present
...
- **AND** emits findings under the new `AK22x` codes
