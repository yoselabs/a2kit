...
ADR 0027, published on `request_scope`.) The fields
are:
...
ADR 0027; it does not redefine that scope's
...
`_CallScopeFilter`, owned by ADR 0027.) The durable
...
`CallRecord` / access-log row of ADR 0027 — it adds
...
ADR 0027. A nested dispatch SHALL report its own
