# substrate-dep-class Specification

## Purpose

The signature splitter's 4th class lets FastAPI `Depends` / `Security` markers pass through to FastAPI's own dependency graph while a2kit retains responsibility for substrate-reserved, container-known, and wire params. Without it, marked params silently fall through to the wire bucket, breaking FastAPI's introspection. The class also makes the MCP-side reject path a clear contract: markers and MCP are incompatible because FastMCP has no equivalent dependency graph.
...
For `substrate="fastmcp"`, the presence of any substrate-dep param SHALL cause `install_substrate_signature` to raise `SubstrateSignatureError("...: parameter(s) [...] carry FastAPI \`Depends\`/\`Security\` markers, which cannot appear on MCP-exposed tools. Remove the marker or scope this tool with expose=('api',).")`.
