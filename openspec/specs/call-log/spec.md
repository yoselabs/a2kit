# call-log Specification

## Purpose
The durable, transport-neutral access-log for tool dispatches (ADR 0027).
Every dispatch mints a request-scope `call_id` (the shared correlation
spine, available even with the call-log off) and the dispatch boundary
auto-captures a span-shaped `CallRecord` (args, raw result value, timing,
principal, span fields) on the dedicated non-streaming `a2kit.calls`
logger. The record is opt-in-persisted as one JSON line per call, with
large bodies content-addressed to sidecars. Enrichment is `debug` logging
correlated by `call_id`, not a dedicated journal API.
