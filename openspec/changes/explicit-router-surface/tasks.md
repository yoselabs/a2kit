...
- [x] 2.4 Add the explicit-tools assertion: `TypeError` if
  `type(self).tools` is missing or not a tuple of callables.
- [x] 2.5 Update tests to assert behaviour driven by the explicit
...
- [x] 2.5.3 In `src/a2kit/app.py`, remove the `on_startup` /
  `on_shutdown` auto-bridge loop (the `cls.__dict__` scan at lines
  149-161).
- [x] 2.5.4 In `App.add_router(r)`, when `r.lifespan` is defined,
...
- [x] 2.5.5 Update tests to assert lifespan composition order and
...
- [x] 2.6.1 Audit every Router subclass to confirm `providers` (when
  ...
  declaration.
- [x] 2.6.2 Document in the `router-conventions` spec that
  ...
  behaviour by spec.)
- [x] 2.6.3 Add a test asserting that a Router with `providers = (P,)`
  installs `P` on the App's container when `add_router` runs.

## 3. A3 — Fold staging decorators into verb-decorator kwargs

- [x] 3.1 Enumerate `@reports(` (and any other staging decorator)
  call sites: `rg "@reports\(" -t py --vimgrep`.
- [x] 3.2 Add `reports: type | None = None` (and any other former
...
- [x] 3.3 For each call site from 3.1, fold the staged argument
  into the adjacent verb decorator's kwarg.
- [x] 3.4 Remove `stage_extra` and `PENDING_EXTRA_ATTR` from
  `src/a2kit/metadata.py`. Remove the verb decorator's flush-loop
  that consumed `_a2kit_pending_extra`.
- [x] 3.5 Remove the `@reports` decorator definition and its
  hosting module at
...
- [x] 4.2 Append this change's surfaces (slug, tools, reports,
  ...
  per surface.
- [x] 4.3 Coordinate with `align-with-pydantic-and-stdlib` author
  on the `A2KitMetaExtras` destination shape; ensure both writers
  land on the same typed keys.
- [x] 4.4 Notify `loud-degrade-everywhere` author that the
