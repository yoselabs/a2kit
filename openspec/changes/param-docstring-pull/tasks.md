...
- [ ] 0.1 Baseline: `uv run pytest --no-cov` green; `make lint` green.
- [ ] 0.2 Capture today's MCP schema and CLI `--help` output for one
...
- [ ] 1.1 Add `src/a2kit/_docstring.py` with a single public helper
...
- [ ] 1.2 Within the section, parse entries of shape
...
- [ ] 1.3 Return an empty mapping on any parse anomaly — never raise.

## 2. Library — wire into `_stamp`

- [ ] 2.1 In `src/a2kit/tool.py::_stamp`, after `_check_return`,
...
- [ ] 3.1 In the MCP input-schema builder, before emitting each
      ...
      `Annotated[...]` metadata.
- [ ] 3.2 In the click-option builder, do the same for the option
      help string.
- [ ] 3.3 Verify nothing changes for parameters that already carry
      an `Annotated[T, Param(description=...)]` wrapper (regression
      anchor from task 0.2).

## 4. Tests

- [ ] 4.1 Unit tests for `_docstring.extract_param_descriptions`:
...
- [ ] 5.2 Update `README.md` / docs section that introduces
...
- [ ] 6.1 `make lint` green.
- [ ] 6.2 `uv run pytest --no-cov` green; new tests under task 4 are
      authored TDD-first (red, then green).
- [ ] 6.3 `openspec validate param-docstring-pull --strict` passes.
- [ ] 6.4 Manual: run the a2web router smoke (or in-repo
