...
- [ ] 1.1 Add a test: construct `McpConfig(instructions="Use entity_* for
...
- [ ] 1.2 Add a test: build an App whose `config.mcp.instructions` is set,
...
- [ ] 2.1 `src/a2kit/config.py`: add `McpConfig.instructions: str | None =
...
- [ ] 2.2 `src/a2kit/packages/mcp/server.py`: at the
...
- [ ] 3.1 New tests from §1 pass.
- [ ] 3.2 Default-None test: an App with no `instructions` set builds a
      server whose instructions behavior is unchanged from today
      (FastMCP default preserved). → `test_default_none_preserves_today`.
- [ ] 3.3 Env-override test: `A2KIT_MCP__INSTRUCTIONS=…` beats a code
      default per ADR 0022 (rides the existing runtime-config env chain).
- [ ] 3.4 Full suite green, output pristine; lint / `ty check src/` /
      a2kit-static / ruff gates green on the two touched files.

## 4. Close out

- [ ] 4.1 Confirm additive/non-breaking: no existing wire shape, field, or
