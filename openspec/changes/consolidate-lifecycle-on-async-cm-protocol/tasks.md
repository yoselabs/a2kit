...
- [ ] 0.2 Inventory existing surfaces this change removes:
  ```bash
...
- [ ] 1.1 `tests/test_app_async_cm.py`:
...
- [ ] 1.2 `tests/test_singleton_type_inference.py`:
...
- [ ] 1.3 `tests/test_singleton_lifecycle_detection.py`:
...
- [ ] 1.4 `tests/test_router_lazy_entry.py`:
...
- [ ] 1.5 `tests/test_lifecycle_topology.py`:
...
- [ ] 1.6 `tests/test_lifecycle_migration_errors.py`:
  ...
  - Router subclass defining `lifespan` classmethod raises `TypeError`
    at `add_router` time
- [ ] 1.7 Run the suite — all six new test files fail with the
  expected red.

## 2. App: `__aenter__` / `__aexit__` implementation

- [ ] 2.1 Add `App.__aenter__` and `App.__aexit__` methods.
...
- [ ] 2.2 Add `self._entered_routers: dict[str, Router] = {}` to
  `App.__init__` (initialized to empty; populated lazily by
  dispatcher).
- [ ] 2.3 `App.__aexit__` unwinds `_entered_routers` in reverse-of-
  enter order, then exits `self._stack`.
- [ ] 2.4 Remove `App.lifespan_cm()` method (was the indirection used
...
- [ ] 3.1 Add `**_kw: Any` to `App.__init__` after documented kwargs;
...
- [ ] 6.2 Remove `a2kit.lifespan` from public re-exports in
  `src/a2kit/__init__.py` (if exported).
- [ ] 6.3 Verify no consumer in `tests/` or `examples/` imports
  `from a2kit.lifespan ...` — migrate any uses.

## 7. Wiring updates

- [ ] 7.1 `build_mcp_server(app)`: wrap `async with app:` in the
  FastMCP `lifespan(server)` slot (per D5).
- [ ] 7.2 CLI runtime: dispatch inside `async with app:` (per D6).
- [ ] 7.3 TestClient: `async with TestClient(app) as client:` —
  internally `await app.__aenter__()` then `await app.__aexit__()`
  on exit (per D7).

## 8. Migration error messages

- [ ] 8.1 Implement each migration `TypeError` per design D8.
  Centralize the messages in `src/a2kit/_migration_errors.py` (new
  module) so the test in task 1.6 can assert exact substrings.

## 9. Spec deltas

- [ ] 9.1 `openspec/changes/consolidate-lifecycle-on-async-cm-protocol/
...
- [ ] 11.1 Port a2web's lifespan body to the new model: extract DB,
      browser pool, LLM client into singletons with `__aenter__`/
      `__aexit__`; remove the imperative `lifespan` function.
- [ ] 11.2 Verify a2web's tests pass against the migrated shape.
- [ ] 11.3 Capture the diff size as proof-of-ergonomics.

## 12. Verify

- [ ] 12.1 All task-1 tests pass.
- [ ] 12.2 Full `make test` green.
- [ ] 12.3 `make lint` green.
- [ ] 12.4 Re-run inventory greps from task 0.2 — `lifespan=` /
      `Router.lifespan` / `singleton(...teardown=)` /
      `a2kit.lifespan` return zero hits in src/.
- [ ] 12.5 README example apps run end-to-end.

## 13. Out-of-scope (deferred)

- [ ] 13.1 Idle-timeout-driven router teardown. Routers stay entered
      until App exit. Future proposal if a real need surfaces.
- [ ] 13.2 Per-tool resource scopes beyond `app.provide(factory)`.
- [ ] 13.3 Health-probe restructure (covered by
