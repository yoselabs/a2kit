...
- [ ] 2.2 Add `test_genuinely_unknown_attribute_raises_attribute_error`:
      - GIVEN a `TestClient` instance
      - WHEN `c.completely_unknown_method` is accessed
      - THEN `AttributeError` is raised (not `TypeError`)
- [ ] 2.3 Ensure `await c.invoke(...)` still works — regression
      guard on the canonical name.

## 3. Spec delta

- [ ] 3.1 Author `openspec/changes/loud-error-on-renamed-test-client-method/
...
- [ ] 4.1 Add migration table row to CHANGELOG.md (Unreleased
      section):
      | before | after |
      |---|---|
      | `await client.call(tool, **kwargs)` | `await client.invoke(tool, **kwargs)` (raises TypeError on `.call` with embedded hint) |
- [ ] 4.2 Note the lack of alias in the entry — explicit signal that
      this is the project's stable rename pattern.

## 5. Verify

- [ ] 5.1 Repro the consumer's exact shape:
      ```python
