...
- [ ] 1.1 **Global-uniqueness collision (the headline RED).** Add a test:
...
- [ ] 1.2 **Per-surface placement does NOT rescue a collision.** Add a
...
- [ ] 1.3 **No-build callability.** Add a test: compose an `App`, never
...
- [ ] 1.4 **Surface-name check available offline.** Add a test: a verb
...
- [ ] 1.5 **Happy path (RED only because the function is absent).** Add a
...
- [ ] 2.1 Extract / introduce a single `resolve_canonical_name(verb)`
...
- [ ] 3.1 Add `validate_composition(app)` that walks app-level verbs +
      ...
      transport.
- [ ] 3.2 Assert **global** canonical-name uniqueness across all verbs.
...
- [ ] 4.2 Confirm `build()` failure carries the offending-pair message
      and that the same App fails identically standalone (shared
      resolver, no drift).

## 5. Verify (GREEN)

- [ ] 5.1 All §1 tests now pass (collision fails loud, disjoint surfaces
      still collide, no-build callability, offline surface check, happy
      path, build backstop).
- [ ] 5.2 Existing `core-composition` / runtime / build tests stay green
      (additive change — uniquely-named Apps see no behavior change).
- [ ] 5.3 Full suite green, output pristine.

## 6. Close out

- [ ] 6.1 lint / `ty check src/` / a2kit-static / ruff gates green on all
      touched files.
- [ ] 6.2 Confirm the resolver is the single shared function the
      forthcoming static dup-name lint rule (`ruff-compatible-lint-codes`)
      will import — no second copy of the resolution precedence.
- [ ] 6.3 Confirm scope boundaries hold: no lint-rule code added here
