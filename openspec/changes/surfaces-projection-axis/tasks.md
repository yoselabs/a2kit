...
- [ ] 1.1 Test: a verb with `surfaces=("mcp","cli")` resolves to LISTED
      on `mcp` and `cli`, ABSENT on `api`. Confirm RED (no `surfaces=`
      kwarg exists yet).
- [ ] 1.2 Test: a verb with `surfaces={"cli": "unlisted"}` resolves to
      UNLISTED on `cli`, ABSENT on `mcp` and `api`. Confirm RED.
- [ ] 1.3 Test: a verb with **no** `surfaces=` resolves to LISTED on
      every registered surface. Confirm RED.
- [ ] 1.4 Test: a dict with an explicit `"listed"`/`"absent"` value
      resolves to that exact state; unlisted keys default to ABSENT.

## 2. Old axes removed (RED)

- [ ] 2.1 Test: `@a2kit.read(expose=("mcp",))` raises `TypeError` whose
      message names `surfaces=` and the mapping. Confirm RED (today it is
      accepted).
- [ ] 2.2 Test: `@a2kit.write(visibility="cli")` raises `TypeError` whose
      message gives the mechanical rewrite `surfaces=("cli",)`. Confirm
      RED.
- [ ] 2.3 Test: `@a2kit.read(visibility="hidden")` raises `TypeError`
      pointing at `surfaces={<surface>: "unlisted"}`. Confirm RED.

## 3. Descriptor carries the resolved matrix (RED)

- [ ] 3.1 Test: `app.tools()[i].surfaces` is a `Mapping` with one entry
      per registered surface, each value ∈ `{"absent","listed","unlisted"}`,
      fully resolved (no `None`). Confirm RED.
- [ ] 3.2 Test: `descriptor.surfaces` is immutable (mutating raises
      `TypeError`). Confirm RED.
- [ ] 3.3 Test: `ToolDescriptor` no longer exposes `expose`. Confirm RED.

## 4. Surfaces honor the matrix (RED)

- [ ] 4.1 Test: a `surfaces=("cli",)` verb is NOT mounted on HTTP (no
      `POST /api/<name>`) and NOT registered on MCP — only on the CLI.
- [ ] 4.2 Test: a `surfaces={"api": "unlisted"}` verb IS callable as
      `POST /api/<name>` but is `include_in_schema=False` (absent from
      OpenAPI). Confirm RED.
- [ ] 4.3 Test: parity — for a mixed-state app, the set mounted on each
...
- [ ] 5.1 Replace `expose=`/`visibility=` with `surfaces=` on the verb
...
- [ ] 5.2 Remove the old kwargs and add the migration-hint `TypeError`
      branches (`src/a2kit/_verb_validators.py`).
- [ ] 5.3 Materialize the resolved matrix onto `ToolDescriptor`
      (`src/a2kit/tool.py`); drop the `expose` field.
- [ ] 5.4 Re-express `_http_mountable` against the matrix and add the
...
- [ ] 6.1 Add a one-minor-version decoration-time shim that recognizes a
...
- [ ] 6.2 Test: the shim maps each row of the `design.md` migration table
      to the correct matrix and warns once. (When the shim is dropped at
      the next minor, these convert to the §2 `TypeError` tests.)
- [ ] 6.3 Deprecation note: record in the change + CHANGELOG that
...
- [ ] 7.1 §1–§4 tests pass.
- [ ] 7.2 Full suite green, output pristine.
- [ ] 7.3 lint / `ty check src/` / a2kit-static / ruff gates green on all
      touched files.

## 8. Close out

- [ ] 8.1 Confirm co-ship: this change lands together with
...
- [ ] 8.2 `openspec validate surfaces-projection-axis --strict` passes.
