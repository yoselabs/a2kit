...
- [ ] 1.1 Create `src/a2kit/packages/di/teardown.py`. Export:
...
- [ ] 1.2 Move the implementations from `container.py` into
...
- [ ] 1.3 In `container.py`:
      ...
      thin delegation.
- [ ] 1.4 Sweep tests/ for any imports of `_build_teardown_edges`
      or internal teardown helpers from `container`. Likely zero;
      update if any.
- [ ] 1.5 Remove `# noqa: A2K014` from `container.py:1`.
- [ ] 1.6 Verify SLOC drops below 500:
      `wc -l src/a2kit/packages/di/container.py`.

## 2. Split tool.py → tool.py + _timeout.py

- [ ] 2.1 Create `src/a2kit/_timeout.py`. Export:
      - `_parse_timeout(value: float | int | str | None) -> float | None`
      - any timeout-related constants (suffix multipliers, etc.)
- [ ] 2.2 Move the `_parse_timeout` implementation from `tool.py`
      into `_timeout.py`. Preserve docstring.
- [ ] 2.3 In `tool.py`, replace the local `_parse_timeout` definition
      with `from a2kit._timeout import _parse_timeout`.
- [ ] 2.4 Sweep tests/ for any imports of `_parse_timeout` from
      `a2kit.tool`. Update to `a2kit._timeout` if found.
- [ ] 2.5 Remove `# noqa: A2K014` from `tool.py:1`.
- [ ] 2.6 Verify SLOC drops below 500:
      `wc -l src/a2kit/tool.py`.

## 3. Mirror test files

- [ ] 3.1 If `tests/packages/di/test_teardown.py` doesn't exist,
...
- [ ] 3.2 If `tests/test__timeout.py` doesn't exist (test mirror
...
- [ ] 4.1 `make test` green; 864 tests still pass (no shifted
      import paths broken).
- [ ] 4.2 `make lint` green; A2K014 produces zero warnings on
...
- [ ] 4.4 Public-API smoke: import `a2kit`, call `read()` and
      `app.singleton(T, factory, teardown=...)` once — confirms
      both modules wire correctly through the public surface.

## 5. Out-of-scope

- [ ] 5.1 Further `tool.py` decomposition (extracting list_view-only
      ...
      again.
- [ ] 5.2 Further `container.py` decomposition (extracting
