...
- **WHEN** the lifespan body runs `await app._resolver.get(SqliteResource)` between `async with app:` and `app.serve_all()`
...
**Reason:** Stdlib `AsyncExitStack` is replaced by a custom per-scope cleanup stack (see `di-scope-cleanup-stack`) to isolate from cpython #137517, MCP SDK #1213, and trio #1243 bug surface. The "no public `lifespan.compose` / `app.use(cm)` / `App(lifespan=...)`" property is preserved by the new cleanup stack contract.

**Migration:**
- The "Public composition surface absent" assertion still holds: there is no `a2kit.lifespan.compose`, no `app.use(cm)`, no `App(lifespan=...)`.
- The mechanism backing the absent surface changes from `AsyncExitStack` to the custom cleanup stack; consumers see no surface change.
