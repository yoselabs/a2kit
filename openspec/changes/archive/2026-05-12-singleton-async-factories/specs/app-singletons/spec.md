...
The `a2kit.App` class SHALL expose `singleton(type_: type[T], factory: Callable[..., T] | Callable[..., Awaitable[T]] | None = None)` that registers a typed factory whose result is cached on the `App` instance and shared across all dispatches that resolve `type_`. When `factory` is omitted, the call SHALL return a decorator that accepts the factory function and completes the registration. The factory MAY be synchronous (`def`) or asynchronous (`async def`); the container SHALL detect the factory's shape at registration time via `inspect.iscoroutinefunction` and remember it on the singleton entry. No new decorator (e.g. `@app.async_resource`) SHALL be introduced; the dual-shape `singleton` method is the entire registration surface for App-scoped cached instances.

#### Scenario: Method form, sync factory
...
#### Scenario: Method form, async factory

- **WHEN** `app.singleton(SqliteResource, build_sqlite_async)` is called where `build_sqlite_async` is `async def`
- **THEN** registration succeeds without raising
- **AND** the container records the factory as async-shaped
- **AND** `SqliteResource` is resolvable via the framework's async resolve path

#### Scenario: Decorator form accepts both shapes

- **WHEN** a function decorated with `@app.singleton(AppState)` is either `def` or `async def`
- **THEN** the function is registered as the cached factory for `AppState` and returned unchanged
- **AND** the container records the factory's shape from `inspect.iscoroutinefunction`

### Requirement: Singletons resolve via the request-scoped container

A registered singleton SHALL be reachable via container resolution exactly like a `provide`-registered type. The container SHALL receive the cached instance on every resolve after the first. For sync-factory singletons, `container.resolve(T)` is synchronous and returns the cached instance directly. For async-factory singletons, the framework's async resolve path (used by dispatch and by `apply_kwargs` in async contexts) SHALL await the factory on first call and cache the result; once cached, `container.resolve(T)` SHALL return the cached instance synchronously without further awaiting.

#### Scenario: Sync singleton resolved twice returns the same instance

- **GIVEN** `app.singleton(AppState, factory)` where `factory` is sync and returns a fresh instance each call
...
#### Scenario: Async singleton awaited on first resolution

- **GIVEN** `app.singleton(SqliteResource, build_sqlite_async)` where `build_sqlite_async` is `async def`
- **WHEN** the framework's async resolve path resolves `SqliteResource` for the first time
- **THEN** the factory coroutine is awaited exactly once
- **AND** the resolved instance is cached on the container

#### Scenario: Async singleton hot-path is sync after first resolution

- **GIVEN** an async-factory singleton that has already been resolved once
- **WHEN** the container resolves the same type again, sync or async
- **THEN** the cached instance is returned without awaiting the factory again
- **AND** no per-type lock is taken on the hot path

#### Scenario: Singleton dependency chains (sync factory)
