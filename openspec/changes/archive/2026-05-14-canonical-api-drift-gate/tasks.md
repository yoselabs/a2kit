...
- [x] 0.1 Baseline: `make test` + `make lint` green. v0.33's
      `tests/test_readme_symbol_drift.py` passes.
- [x] 0.2 Read the existing drift gate to understand its parse
      shape — extension should reuse the same scanning approach
      where possible.

## 1. Define the canonical-type allowlist

- [x] 1.1 In `tests/test_readme_symbol_drift.py` (or a new helper
      module), declare:
      ```python
      CANONICAL_TYPES = {
      ...
      ```
- [x] 1.2 Resolve each entry at test-import time to a live type
...
- [x] 2.1 Walk fenced ```python``` blocks in `README.md`. For each
...
- [x] 2.2 For each pair, assert `hasattr(canonical_types[obj_name],
      attr_name)`. Collect failures into a list and fail with one
      structured error message listing every drift hit.
- [x] 2.3 Handle the variable-name-to-type mapping with a small
...
- [x] 3.1 Create `tests/test_canonical_apis.py`. Fixture: a tiny
      single-router app with one read tool.
- [x] 3.2 Exercise the documented call shapes end-to-end:
...
- [x] 3.3 Each call shape is exactly one test. Failure messages
      point at the README section claiming the shape.

## 4. Spec delta — docs-code-parity

- [x] 4.1 Author `openspec/changes/canonical-api-drift-gate/
...
- [x] 5.1 The existing `make lint` already runs
...
- [x] 5.2 Confirm both tests run in the lint stage; failure breaks
      `make lint`.

## 6. Verify

- [x] 6.1 `make test` green; the two new tests pass.
- [x] 6.2 `make lint` green; the new lint stages pass.
- [x] 6.3 Smoke: temporarily rename `TestClient.invoke` to
...
- [x] 7.1 Static type-checking of README code blocks. Too brittle;
      typers diverge across Python versions and FastMCP versions.
- [x] 7.2 Extending to `OPERATIONAL_CONTRACTS.md` and
