...
a2kit SHALL provide a module `a2kit.packages.dispatch.request_scope` exposing five callables: `publish`, `get`, `try_get`, `all_seeds`, `reset`, plus the exception class `RequestScopeMissing`. The module SHALL carry exactly one `ContextVar` (module-private, not re-exported through any `__all__`). All request-scoped values traveling from substrate boundary code to dispatch stages SHALL travel through this bridge — per-type `_<x>_bridge.py` modules and per-value ContextVars SHALL be retired (or kept only as deprecation shims for one release).

#### Scenario: Bridge module exposes exactly the documented surface

- **WHEN** code does `from a2kit.packages.dispatch import request_scope`
- **THEN** `request_scope.__all__` equals `("publish", "get", "try_get", "all_seeds", "reset", "RequestScopeMissing")`
