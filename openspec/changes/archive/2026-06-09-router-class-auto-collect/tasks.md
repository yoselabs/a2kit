...
- [x] 1.1 Write a failing test: define a `Router` subclass with two
      ...
      'tools'").
- [x] 1.2 Write a failing test: a decorated method on a tuple-free router
      is callable end-to-end (dispatch resolves it, DI injects its typed
      kwargs). Confirm RED.
- [x] 1.3 Implement `Router.__init_subclass__` decorator-marker
...
- [x] 1.4 Remove the `tools: ClassVar[tuple]` attribute, its
...
- [x] 2.1 Write a test proving collection is marker-driven, not
...
- [x] 2.2 Write a test for inheritance MRO: a subclass that inherits a
...
- [x] 4.1 Write a failing test: an in-class `@a2kit.enricher`-marked
...
- [x] 4.2 Add the `@a2kit.enricher` marker to the verb-decorator surface
      and collect enricher-marked methods in `__init_subclass__` (same
      hook as §1.3). Make §4.1 GREEN.
- [x] 4.3 Retire the post-construction `@router.enricher` instance
      ...
      marked-method form.
- [x] 4.4 Confirm chain order is unchanged: per-tool inline → router
      enrichers (declaration order) → app enrichers → defect quarantine;
      first non-None `AppError` wins.

## 5. Spec + ADR doc tasks

- [x] 5.1 Land `specs/router-conventions/spec.md` (MODIFIED): drop the
...
- [x] 5.2 **Update ADR 0002 status linkage (doc task).** Edit
...
- [x] 5.3 Update `src/a2kit/routers.py` examples and any author-facing
...
- [x] 6.1 New tests from §1–§4 pass.
- [x] 6.2 Existing router-conventions / dispatch / enricher tests stay
      green after migration to the tuple-free + in-class-enricher shape.
- [x] 6.3 `public-api-tier-snapshot` static derivation still produces the
      same tool set (decorators remain AST-visible without the tuple).
- [x] 6.4 Full suite green, output pristine; lint / `ty check src/` /
      a2kit-static / ruff gates green on all touched files.

## 7. Close out

- [x] 7.1 Confirm co-ship readiness with `native-tree-homomorphism`,
      `surfaces-projection-axis`, and `app-as-peer-root` (one Wave 2
      breaking release; one consumer migration table).
- [x] 7.2 Verify the consumer migration is mechanical: a2atlassian /
