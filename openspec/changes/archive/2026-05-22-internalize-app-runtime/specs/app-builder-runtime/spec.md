...
**Reason**: `AppBuilder` is removed; composition happens on `a2kit.App`
directly. Replaced by "Composition happens on a mutable App".

**Migration**: Replace `a2kit.AppBuilder("svc")` with `a2kit.App("svc")`.
The composition verbs are unchanged.

### Requirement: The built App is a mutation-free runtime

**Reason**: There is no consumer-visible runtime type. `a2kit.App` is
the mutable composition object; the sealed runtime is internal.

**Migration**: Hold the `a2kit.App` returned by `a2kit.App(...)`. Do
not expect a separate sealed type — pass the `App` straight to a
finisher.

### Requirement: build() is the seal point

**Reason**: There is no public `build()`. Finishers (`run`,
`build_mcp_server`, `testing.client`) seal the `App` internally.
Replaced by "Finishers seal the App".

**Migration**: Delete the `.build()` call — `a2kit.AppBuilder(...).build()`
becomes `a2kit.App(...)`. Hand the `App` directly to `a2kit.run`,
`build_mcp_server`, or `a2kit.testing.client`.

### Requirement: Direct App construction is rejected with a migration hint

**Reason**: Direct construction of `a2kit.App` is now the supported and
only way to obtain an App.

**Migration**: `a2kit.App("svc")` is correct — no builder, no `build()`.
