...
- [x] 1.1 Write a test: define `class Kay(a2kit.App): name = "kay"` with an
...
- [x] 1.2 Write a test: an App class with `providers = (ConnStore, (Database,
...
- [x] 1.3 Write a test: `@a2kit.enricher def on_timeout(self, exc:
...
- [x] 2.1 Write a test: `@app.read def health` resolves to canonical name
...
- [x] 4.1 Write a test: `def configure_api(self, api): ...` on the App class
      ...
      coverage. RED.
- [x] 4.2 Write a test: the bootstrap escape (form (b)) still works —
...
- [x] 4.3 Document/guard form (a): a class-body `@app.api.get(...)` is
...
- [x] 5.1 Make `App` collect class-body `@a2kit`-marked verbs + enrichers via
      `__init_subclass__` (reuse the `router-class-auto-collect` mechanic).
      App-level verbs carry no slug → bare-leaf canonical name.
- [x] 5.2 Read `name` / `providers` / per-surface config from class attrs;
      install `providers` the way Router providers are installed.
- [x] 5.3 Add `routers` ClassVar handling (reference-composition); compose
...
- [x] 5.4 Add the `configure_api` / `configure_mcp` / `configure_cli` hooks
...
- [x] 5.5 `Kay().serve()` entry-point path (instantiate then run).

## 6. Verify (GREEN)

- [x] 6.1 All §1–§4 tests pass.
- [x] 6.2 Symmetry check: an App and a Router authored side by side share
      the same authoring members (verbs, enrichers, configure_* hooks,
      class-attr config) per the roots-side-by-side table.
- [x] 6.3 Full suite green, output pristine; coverage holds.
- [x] 6.4 Co-ship check: the migration table for Wave 2 includes the
      `add_router → routers=` and `app.api.get class-body → configure_api
      / (b) escape` rows.

## 7. Close out

- [x] 7.1 lint / `ty check src/` / a2kit-static / ruff gates green on all
      touched files.
- [x] 7.2 Confirm Wave-1 dependency satisfied (`cli-as-surface` landed: the
