...
- [x] 0.1 Baseline: `make lint` and `make test` green at HEAD.
- [x] 0.2 Confirm `descriptor.format_hint` is populated for every
...
- [x] 1.1 Add `async def call_wire(self, tool_name: str, *,
...
- [x] 1.3 Verify the method docstring documents (a) return-type
...
- [x] 2.5 Scenario "capture surfaces populate": a tool that emits
      ...
      populated.
- [x] 2.6 Scenario "invoke unchanged": same tool as 2.2; assert
      `await c.invoke(...)` still returns the `list[Task]` Python
      object, not the TSV string. Regression guard.

## 3. Docs

- [x] 3.1 Update `src/a2kit/packages/testing/client.py` module
      docstring with a `call_wire` example next to the existing
      `render_as` example. Show the JSON-vs-TSV difference in two
      lines.
- [x] 3.2 If the README has a "Testing tools" section that lists
      `invoke` / `render_as`, append a one-line `call_wire` entry.

## 4. Validation

- [x] 4.1 `openspec validate testing-client-wire-payload --strict`
      passes.
- [x] 4.2 `make lint` green.
- [x] 4.3 `make test` green; the five new scenarios in 2.x land.

## 5. Downstream signal (out of scope for this change)

- [x] 5.1 Note in the a2web feedback round-6 log that friction 2
