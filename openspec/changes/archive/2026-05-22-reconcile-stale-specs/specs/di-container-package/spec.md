...
**Reason**: This requirement mandates `Container._snapshot()` and `Container._restore(snapshot)` as a test-only isolation seam. That seam does not exist on the `Container` class. The post-seal test-override mechanism (`_override` / `_snapshot` / `_restore`) was deleted; test overrides are done by composition-root re-registration — constructing a fresh `App` and calling `provide` with the fake (last-write-wins). No snapshot/restore pair backs any current capability.

**Migration**: For test-time dependency swaps, construct a fresh `App` and call `app.provide(T, fake)` before handing the App to a finisher. The fake wins by last-write-wins re-registration. There is no `Container._snapshot` / `_restore` to call.
