...
`DecoratorSurface[R]` SHALL be a generic Template class (R = the registration dataclass type) owning a `registrations: tuple[R, ...]` accumulator plus `_decorator(kind, **kwargs)` and `_wrap(fn, registration)` skeletons. `McpSurface` and `ApiSurface` SHALL extend `DecoratorSurface` with their concrete registration type; they SHALL NOT carry their own registration accumulators.
