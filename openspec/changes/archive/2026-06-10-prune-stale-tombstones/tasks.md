...
- [x] 1.1 Amend `AGENTS.md` §1 ("No backward compatibility shims") with the
...
- [x] 1.2 Record the keep-list (in-flight cluster: ADR 0028 positional
...
- [x] 2.1 Apply the spec deltas: REMOVE `app-lifecycle` lifespan req +
...
- [x] 2.2 `openspec validate prune-stale-tombstones --strict` passes.

## 3. Update tests to the post-sweep behavior (RED → watch fail)

- [x] 3.1 Rewrite the raise-assertion tests for the swept tombstones to
...
- [x] 3.2 Run them; confirm they fail against the still-present tombstones
      (the bespoke hints still fire) for the right reason.

## 4. Sweep the tombstones (GREEN)

- [x] 4.1 `src/a2kit/app.py`: delete the special-cased `lifespan` / `debug` /
...
- [x] 4.2 `src/a2kit/__init__.py`: remove the `@a2kit.tool` entry from
      `_REMOVED`.
- [x] 4.3 `src/a2kit/packages/dispatch/substrate.py`: delete the
      `Substrate` raising `__getattr__`.
- [x] 4.4 Delete `src/a2kit/packages/cli/context.py` (the tombstone module)
      and its `import_acyclicity` / layer-manifest references.
- [x] 4.5 `src/a2kit/packages/testing/snapshots.py`: delete the
      `TOONSnapshotExtension` tombstone.
- [x] 4.6 Keep untouched: positional `App(...)`, `App.add_router`,
      `TestClient.override` + renamed-method tombstones (in-flight horizon).

## 5. Reconcile the drift gates (sweeping changes `hasattr`)

- [x] 5.1 Swept names no longer resolve, so the README / spec / docs
...
- [x] 5.2 Confirm `tests/test_spec_symbol_drift.py` allowlist no longer
      needs the swept tombstone names (remove any that were allowlisted as
      tombstone-migration targets, e.g. `a2kit.tool`).

## 6. Verify

- [x] 6.1 Full suite green; `make lint` green (ruff / ty / a2kit-lint /
      all three drift gates / openspec-validate / surface snapshots).
- [x] 6.2 `public-api-tier-snapshot` unaffected (no `_LAZY_ATTRS` live
