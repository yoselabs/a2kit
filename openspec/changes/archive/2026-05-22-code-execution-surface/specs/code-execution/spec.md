...
When an a2kit MCP server is built with code mode enabled — the default — the server SHALL install FastMCP's `CodeMode` transform. The transform collapses the listed tool catalog: `list_tools` returns only the discovery and execute meta-tools (`search`, `get_schema`, `execute`). The server's real tools remain callable by name but are no longer enumerated. The tool author writes nothing to obtain this surface.

#### Scenario: Default build installs code mode

- **GIVEN** an `a2kit.App` with one or more registered tools
...
Code execution SHALL be disableable by a single toggle. `build_mcp_server(app, code_mode=False)` SHALL NOT install the transform. The `serve` command SHALL accept `--code-mode-off` and map it to `code_mode=False`. With code mode off, `list_tools` returns the full real tool catalog and no `execute` tool is present.

#### Scenario: Build with code mode off
...
#### Scenario: serve honours the flag
