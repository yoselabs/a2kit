...
- [x] 0.2 Re-run the audit greps from `CLAUDE.md` to confirm the
...
- [x] 1.1 `src/a2kit/tool.py::_compute_report_schema` — add WARN log
      ...
      class+message.
- [x] 1.2 `src/a2kit/packages/health/__init__.py::_version` — replace
...
- [x] 1.3 `src/a2kit/packages/lint/runtime.py:43` — drop the empty-
...
- [x] 2.1 `src/a2kit/packages/mcp/server.py:402-405` — remove the
...
- [x] 2.2 `src/a2kit/packages/cli/runtime.py:115` — replace
...
- [x] 3.1 `src/a2kit/app.py::App.__init__` — add `**_kw: Any` after
...
- [x] 3.2 `src/a2kit/routers.py::Router.__init__` — same pattern.
      (Routers don't currently take subclass-supplied kwargs, but
      the guard locks in the convention.)

## 4. Tests

- [x] 4.1 Add `tests/test_no_silent_fallbacks.py` covering:
      ...
      - Lint runtime entry raises when `.name` is missing on a
        descriptor
- [x] 4.2 Add `tests/test_app_init_kwarg_guard.py`:
      - `a2kit.App("a", unknown_kwarg=True)` raises `TypeError`
      - The error message mentions `unknown_kwarg` and the
        CHANGELOG
- [x] 4.3 Confirm no existing test relied on the silent-fallback
      behaviour (grep for `_compute_report_schema` callers in
      tests).

## 5. Spec delta

- [x] 5.1 Author `openspec/changes/audit-loud-failure-discipline/
...
- [x] 6.1 Add migration table entries to CHANGELOG.md Unreleased
...
- [x] 7.1 `make test` green; new tests pass.
- [x] 7.2 `make lint` green.
- [x] 7.3 Re-run audit greps from CLAUDE.md — the patterns flagged
