...
- [x] 1.1 Move `typer>=0.25,<1` from dev `optional-dependencies`
...
- [x] 1.3 Add a top-level import test: `python -c "import a2kit"`
...
- [x] 2.1 Create `src/a2kit/packages/cli/_field_to_typer.py`
...
- [x] 2.2 Unit-test the adapter standalone:
      `tests/packages/cli/test_field_to_typer.py`.

## 3. Rewrite `builder.py`

- [x] 3.1 Replace `src/a2kit/packages/cli/builder.py` body with a
      Typer-based implementation (~70-100 LOC target).
- [x] 3.2 Preserve `LazyGroup`-equivalent behavior for `serve`
      (Typer's `add_typer` with a callback factory, or a Typer
      app whose `serve` subcommand triggers fastmcp import only
      on invocation).
- [x] 3.3 Preserve top-level `--no-reports` / `--no-events` flags.
- [x] 3.4 Preserve `<app> health` shorthand when `_health.enabled`.
- [x] 3.5 Preserve docstring-to-help mapping (PEP 257 dedent +
      markdown strip for CLI long-help).
- [x] 3.6 Build per-tool commands via a `make_typer_command(app, fn, router)`
      adapter (the spike prototype shape).

## 4. Format routing wiring

- [x] 4.1 Inside each Typer command callback, invoke
      `format_response(value, format_hint=descriptor.format_hint).data`
      and emit via `typer.echo` exactly once.
- [x] 4.2 Preserve the per-tool `--format {auto,json,tsv,page-tsv}`
      option and `--schema` flag wiring.
- [x] 4.3 No double-encoding: command body returns `None` after
      explicit echo (matches the spike Q3 finding).

## 5. Exception pipeline wiring

- [x] 5.1 Set `pretty_exceptions_enable=False` on the top-level
      Typer app.
- [x] 5.2 Wrap the tool fn with `_wrap_with_enricher` (existing
      helper) BEFORE handing to `make_typer_command`, so enrichers
      see the exception before Typer.
- [x] 5.3 Verify Click-style UsageError messages still appear for
      missing required options and bad types.

## 6. Connection synthesis

- [x] 6.1 In `make_typer_command`, when `wire_input_params` reports
...
- [x] 6.2 The wrapper strips `connection` from kwargs before
      calling the underlying tool unless the tool itself declares
      `connection` (matches current behavior in
      `_make_tool_command`).
- [x] 6.3 Test: a tool with no `connection` parameter still exposes
...
- [x] 7.1 Create `src/a2kit/schema.py` containing only
...
- [x] 7.2 Add `"schema"` to `_LAZY_MODULES` in
...
- [x] 7.6 Replace the per-tool `--schema` flag handler in
      `builder.py` line 295 area: import from `a2kit.schema`.
- [x] 7.7 Delete `src/a2kit/packages/cli/schemas.py` entirely.
- [x] 7.8 Verify with `grep -rn "from.*cli.schemas\|cli\.schemas"
      src/ tests/ examples/` — zero matches. No surviving import.
- [x] 7.9 Verify cold-start invariant: `python -X importtime -c
      'import a2kit'` does NOT show `a2kit.schema` in the loaded
      modules tree.

## 8. Test suite update

- [x] 8.1 Audit `tests/packages/cli/` (and any other tests that
      exercise the CLI builder). Update assertions for Typer-style
      `--help` text and error messages where they differ from
      Click-only output.
- [x] 8.2 Add a test for the JSON-string body-model UX:
      `<app> tool --body '{"k":"v"}'` decodes correctly.
- [x] 8.3 Add a test that confirms `pretty_exceptions_enable=False`
      effective behavior: an unenriched exception prints a plain
      message, not a Rich-styled traceback.
- [x] 8.4 Run the full `tests/` suite green; no regression beyond
      D3 is acceptable.

## 9. Docs

- [x] 9.1 `CHANGELOG.md`: add a breaking-shape note for body-model
      CLI users (JSON-string form). Even though the in-repo blast
      radius is zero, the change is observable for downstream
      authors.
- [x] 9.2 `README.md`: if any example shows body-model CLI usage,
      update the example. (Audit: likely zero updates needed.)
- [x] 9.3 Write an ADR documenting the Typer migration decision at
...
- [x] 10.1 Run:
      ...
      each).
- [x] 10.2 Same audit for `: BaseModel` outside of model definitions:
      ```
      grep -rEn "def [a-z_]+\([^)]*\*,[^)]*:\s*[A-Z][A-Za-z]+\)" ...
      ```
      Cross-check against types that are pydantic models — those
      are the affected callers.
- [x] 10.3 Record the audit result in the PR description.

## 11. Validate

- [x] 11.1 Run `openspec validate replace-cli-builder-with-typer --strict`;
      iterate until clean.
- [x] 11.2 Run the full test suite green.
- [x] 11.3 Re-measure cold-start via `python -X importtime` on a
