...
- [ ] 1.1 Add a test asserting `SurfaceKind` has exactly the members
...
- [ ] 1.2 Add `SurfaceKind(enum.Enum)` and the `kind: ClassVar[SurfaceKind]`
...
- [ ] 1.3 Add a test asserting `McpSurface.kind == SurfaceKind.NETWORK`
...
- [ ] 2.1 Add a test: `isinstance(CliSurface(), Surface)` is `True`,
      `CliSurface().name == "cli"`, `CliSurface.kind == SurfaceKind.LOCAL`.
      Confirm RED (no `CliSurface` yet).
- [ ] 2.2 Add `CliSurface` (`name = "cli"`, `kind = LOCAL`, empty
...
- [ ] 3.1 Add a test: `CliSurface().bind(runtime)` returns a
...
- [ ] 3.2 Move the `build_full_cli` body into `CliSurface.bind` (call the
...
- [ ] 3.3 Add a parity test: assemble the CLI via `CliSurface().bind`
      ...
      App. GREEN.
- [ ] 3.4 Add a test: with `A2KIT_TOOLS` set to a subset, the
...
- [ ] 4.1 Add a test: an App with `cli_extras` (Click `add_cli(...)`
...
- [ ] 4.2 Add a test: a root command exposing no callable `add_command`
...
- [ ] 4.3 Move the Wave 0 `add_command` guard (`builder.py:594-608`)
...
- [ ] 5.1 Add a test: `app.cli` returns a `CliSurface`, a second access
      returns the same instance (idempotent). Confirm RED (no `cli`
      property).
- [ ] 5.2 Add the `App.cli` lazy property (`importlib`-load `CliSurface`),
      with a `_cli` backing slot alongside `_api` / `_mcp` in `__init__`,
      mirroring the `api` / `mcp` shape. GREEN.
- [ ] 5.3 Add a cold-start test: after `import a2kit`, reaching the
...
- [ ] 6.1 Confirm (or add) a test that the default
      `runtime.surfaces.names()` is still `("mcp", "api")` — the LOCAL
      CLI surface does NOT join the network mount set. GREEN.
- [ ] 6.2 Confirm the top-level CLI entry point (`a2kit.run`) is built
      via `CliSurface().bind(runtime)` and that `CliSurface.kind ==
      LOCAL` keeps it out of the parent ASGI app. GREEN.

## 7. Verify (GREEN)

- [ ] 7.1 New tests from §1–§6 pass.
- [ ] 7.2 Full existing CLI suite stays green — assembled CLI behavior,
      tool names, flags, and output shapes are byte-for-byte unchanged.
- [ ] 7.3 Full suite green, output pristine; `import a2kit` cold-start
      unchanged (no `typer` / `fastmcp` pulled).

## 8. Close out

- [ ] 8.1 lint / `ty check src/` / a2kit-static / ruff gates green on
      all touched files.
- [ ] 8.2 Confirm the vendored-click shim exists in exactly one place
      (`CliSurface.bind`); no stray copy in the builder.
- [ ] 8.3 Forward-compat seam: Wave 2 (`native-tree-homomorphism`,
