"""The a2kit `CodeMode` transform — capability gate, marshalling, type-checking.

`A2kitCodeMode` is FastMCP's experimental `CodeMode` plus three a2kit
additions: a `destructive`-tool capability gate, dict→dataclass marshalling
of `call_tool` results, and pre-execution type-checking via
`A2kitSandboxProvider`. `build_code_mode_transform` is the single entry
point; it fails fast when the Monty sandbox runtime is absent.
"""
