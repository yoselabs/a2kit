...
``_LAZY_ATTRS`` dict of ``name -> (module, attr)`` for re-exports and an
optional ``_LAZY_MODULES`` dict of ``name -> module`` for submodule aliases —
...
def lazy_attr(
    ...
    *,
    ...
    """Return a PEP 562 ``__getattr__`` resolving ``attrs`` / ``modules``."""
    modules = modules or {}
...
__all__ = ["LazyAttrSpecs", "LazyModuleSpecs", "lazy_attr", "lazy_dir"]
