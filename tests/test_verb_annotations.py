...
def test_read_name_kwarg_rejected() -> None:
    ...
    with pytest.raises(TypeError, match="name"):
        a2kit.read(name="custom")  # type: ignore[call-arg]  # ty: ignore[unknown-argument]  # why: test passes a removed kwarg to assert the migration-hint TypeError


def test_write_name_kwarg_rejected() -> None:
    with pytest.raises(TypeError, match="name"):
        a2kit.write(name="custom")  # type: ignore[call-arg]  # ty: ignore[unknown-argument]  # why: test passes a removed kwarg to assert the migration-hint TypeError


def test_list_name_kwarg_rejected() -> None:
    with pytest.raises(TypeError, match="name"):
        a2kit.list_("id", name="custom")  # type: ignore[call-arg]  # ty: ignore[unknown-argument]  # why: test passes a removed kwarg to assert the migration-hint TypeError
