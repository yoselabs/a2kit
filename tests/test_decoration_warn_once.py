...
def test_l2_resolve_return_annotation_warns_once(caplog: pytest.LogCaptureFixture) -> None:
    ...
    def fn() -> "Unresolved":  # type: ignore[name-defined]  # noqa: F821, UP037  # ty: ignore[unresolved-reference]  # why: annotation references a deliberately-undefined name to trigger get_type_hints failure
...
def test_l3_derive_selectable_fields_warns_once(caplog: pytest.LogCaptureFixture) -> None:
    ...
    def fn() -> "list[Unresolved]":  # type: ignore[name-defined]  # noqa: F821, UP037  # ty: ignore[unresolved-reference]  # why: annotation references a deliberately-undefined name to trigger get_type_hints failure
        return []
...
class _StubServer:
    ...
    async def get_tool(self, _name: str) -> Any:
        ...
        t = _Tool()
        t.meta = self._meta  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]  # why: stub object exposes attributes only at runtime; static checker can't see them
        return t
...
async def test_l4_listview_get_tool_failure_warns_once(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="a2kit.packages.mcp.listview"):
        out1 = await mw.on_call_tool(ctx, _identity_call_next)  # ty: ignore[invalid-argument-type]  # why: ty's narrowed parameter type rejects this call; runtime accepts duck-typed/stub argument
        out2 = await mw.on_call_tool(ctx, _identity_call_next)  # ty: ignore[invalid-argument-type]  # why: ty's narrowed parameter type rejects this call; runtime accepts duck-typed/stub argument
...
async def test_l4_listview_projection_failure_warns_once(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="a2kit.packages.mcp.listview"):
        out1 = await mw.on_call_tool(ctx, _seed_call_next)  # ty: ignore[invalid-argument-type]  # why: ty's narrowed parameter type rejects this call; runtime accepts duck-typed/stub argument
        out2 = await mw.on_call_tool(ctx, _seed_call_next)  # ty: ignore[invalid-argument-type]  # why: ty's narrowed parameter type rejects this call; runtime accepts duck-typed/stub argument
