...
def test_save_uses_tempfile_then_rename(db_store: ConnectionStore[DbInfo], monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    db_store.save(DbInfo(key=("a", "b", "c"), dsn="sqlite://"))
    assert len(seen_tmp_writes) == 1
    assert seen_tmp_writes[0].endswith(".tmp")
    assert seen_tmp_writes[0].startswith(".a-b-c.")


def test_save_replaces_existing_atomically(db_store: ConnectionStore[DbInfo]) -> None:
    """Two saves to the same key produce one final file with the latest content."""
    db_store.save(DbInfo(key=("a", "b", "c"), dsn="sqlite:///v1.db"))
    db_store.save(DbInfo(key=("a", "b", "c"), dsn="sqlite:///v2.db"))
    files = list(db_store.config_dir.glob("*"))
    # Exactly one final file, no leftover tmp.
    assert len(files) == 1
    assert files[0].name == "a-b-c.toml"
    assert db_store.load(("a", "b", "c")).dsn == "sqlite:///v2.db"


def test_chmod_sets_owner_only(db_store: ConnectionStore[DbInfo]) -> None:
    path = db_store.save(DbInfo(key=("a", "b", "c"), dsn="sqlite://"))
